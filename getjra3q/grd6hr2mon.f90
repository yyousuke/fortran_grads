!c  
!c  grd6hr2mon.f90
!c  2017/02/09 Yamashita (from grd6hr2mon.f90)
!c
!c  calculate monthly mean data from 6hr data
...
program main
  ...
  logical              :: onefile    !! t: output 1 file on, f: off
  !c+++ from file
  ...
  do iyy = nsyy, neyy
    ...
    do imm = nsm, nem
      ...
      do it = ista, iend
        ...
        if (iflag == nt) then
          irec = irec + 1
          !c+++ calc monthly mean data
          call TIMEave(nx, ny, nz, nt, undef8, din, dout)
          !c+++ shift x-grids
          if (xshift /= 0) then
    ...
    enddo !! imm
    call fclose(21)
    if (.not. onefile) call fclose(51)
  enddo !! iyy
  if (onefile) call fclose(51)
  deallocate(dout)
