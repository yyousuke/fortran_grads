...
!c  program gtcol
!c  [history]
!c  2019/03/01 Yamashita: first ver. (from gtdob ver. 2017/08/04)
!c
!c  output Column mean amount [atm/m2] from vmr for p-, sig-, and eta-levels
!c
!c  internal: module common_col
...
program gtcol
  ...
  use common_col, only: setup_cnst, h
  ...
  real(kind=r4b), allocatable :: gda(:,:,:)     !! [vmr]
  ...
  real(kind=r4b), allocatable :: colm(:,:)      !! [atm/m2]
  !c+++ I/O unit number
...
!c===
  ...
  allocate(p(kmax), gda(imax,jmax,kmax))
...
!c===
  ...
  do while (1 == 1)
    ...
    !c+++ vmr ==> colm
    call colout(imax, jmax, kmax, k1, k2, rmiss, gda, pres, colm, factuni)
  ...
  !c+++ deallocate
  deallocate(ps, ps4, p, gda, pres, colm)
...
subroutine colout(imax, jmax, kmax, k_l, k_h, rmiss, gda, pres, colm, factuni)
  ...
  real(kind=r8b), intent(in)    ::  factuni             !! factor of output data unit
  !c+++ [output]
  real(kind=r4b), intent(out)   :: colm(imax,jmax)      !! column mean (/m2)
  !c+++ [internal work]
  real(kind=r8b)                :: fact                 !!
  ...
  fact = NA / mair * factuni
  call vint(imax, jmax, kmax, k_l, k_h, rmiss, gda, p, colm, GR, fact)

  return
end subroutine colout
...
subroutine getparms
  ...
  call get_parms('titl', 'column mean', htitl, ios)
  call get_parms('unit', 'atm/m2', hunit, ios)
  !c+++ start/end time
  ...
  call c2var(factuni, '(1pe15.5)', hval)
  !c+++ data format
...
subroutine xabort
  ...
  write(6, '(a)') '------------------------------------------'
  write(6, '(a)') ' '
  write(6, '(a)') 'ex. Column mean (atm/m2) for p-levels: '
  write(6, '(a)') '    gtcol -i inp_P -o colm'
  write(6, '(a)') ' '
  write(6, '(a)') 'ex. Column mean (atm/cm2) for p-levels: '
  write(6, '(a)') "    gtcol -i inp_P -o colm -fact 1.d-4 -unit 'atm/cm2' "
  write(6, '(a)') ' '
  write(6, '(a)') 'ex. Partial column (atm/m2) for p-levels:'
  write(6, '(a)') '    gtcol -i inp_P -o colm -k1 9 -k2 10'
  stop 2
