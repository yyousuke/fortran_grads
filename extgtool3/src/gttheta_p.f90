...
!c  gttheta_p.f90
!c  [history]
!c  2010/07/28 Yamashita: T==>THETA conversion for p-level input
!c  2013/03/03 Yamashita: use gtool header to determine array size
!c  2013/03/03 Yamashita: add sigma-/eta- level input
!c  2016/01/28 Yamashita: add gtopen, set_plevs
!c  2016/01/28 Yamashita: add inv, apnd options
...
!c
!c  inv=f: T==>THETA conversion for p-, sig-, and eta-levels
!c  inv=t: THETA==>T conversion for p-, sig-, and eta-levels
!c
!c  internal: module common_theta
!c            subroutine setup_cnst
!c  internal: module common_theta
!c  internal: subroutine getparms
!c    internal: subroutine xabort
!c  internal subroutine conv_t2th
!c  internal subroutine conv_th2t
...
module common_theta
  ...
  use common_const, only: cnst_pre00, cnst_kappa, cnst_h, const_setup
...
program gttheta_p
  ...
  use common_theta, only: setup_cnst, h
...
!c===
  ...
  allocate(T(imax,jmax,kmax), TH(imax,jmax,kmax))
...
!c===
  ...
  do while (1 == 1)
    ...
    !c+++ read temperature/potential temperature
    ...
    if (.not. oinv) then
      ...
      !c+++ convert: T, P ==> THETA
      call conv_t2th(imax, jmax, kmax, rmiss, T, pres, TH)
      dout(1:imax,1:jmax,1:kmax) = TH(1:imax,1:jmax,1:kmax)
    else
      TH(1:imax,1:jmax,1:kmax) = din(1:imax,1:jmax,1:kmax)
      !c+++ convert: THETA, P ==> T
      call conv_th2t(imax, jmax, kmax, rmiss, TH, pres, T)
      dout(1:imax,1:jmax,1:kmax) = T(1:imax,1:jmax,1:kmax)
    endif
...
!c
!c=====
subroutine conv_t2th(imax, jmax, kmax, rmiss, T, pres, TH)
  use common_theta, only: P00, rkappa
  implicit none
  !c+++ [input]
  ...
  !c+++ [output]
  real(kind=r8b), intent(out)   :: TH(imax,jmax,kmax)   !! THETA (K)
  !c+++ [internal work]
...
!c
!c=====
subroutine conv_th2t(imax, jmax, kmax, rmiss, TH, pres, T)
  use common_theta, only: P00, rkappa
  implicit none
  !c+++ [input]
  integer(kind=i4b), intent(in) :: imax, jmax, kmax     !! x-, y-, z-sizes
  real(kind=r8b), intent(in)    :: rmiss                !! missing value
  real(kind=r8b), intent(in)    :: TH(imax,jmax,kmax)   !! THETA (K)
  real(kind=r8b), intent(in)    :: pres(imax,jmax,kmax) !! p (hPa)
  !c+++ [output]
  real(kind=r8b), intent(out)   :: T(imax,jmax,kmax)    !! T (K)
  !c+++ [internal work]
  integer(kind=i4b)             :: i, j, k
  ...
  do k = 1, kmax
    do j = 1, jmax
      do i = 1, imax
        !c+++ T = TH * (p/Ps)**KAP
        if (TH(i,j,k) /= rmiss.and.pres(i,j,k) /= rmiss) then
...
subroutine getparms
  ...
  !c+++ inv
  call get_parms('inv', 'f', hval, ios)
  call c2var(oinv, '(l1)', hval)
  !c+++ apnd
