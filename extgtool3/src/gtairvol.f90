...
!c  program gtairvol
!c  [history]
!c  2026/03/07 Yamashita: first ver. (from 2017/08/04 gtairmassv.f90 by Yamashita)
!c
!c  gridvox volume of air in layer
!c
!c  airvol = area [m^2] * height [m]
!c
!c  internal: module common_airmass
!c            subroutine setup_cnst
!c  internal: subroutine volume
...
!c===
  ...
  do while(1 == 1)
    ...
    !c+++ air volume
    call volume(imax, jmax, kmax, rmiss, din, pres, lat, dout)
...
!c
!c=====
subroutine volume(imax, jmax, kmax, rmiss, ndm, pres, lat, dout)
...
&            abs(0.5d0 * (cos(xlatm(j)) + cos(xlatm(j+1))) * (xlatm(j) - xlatm(j+1)))
      do i = 1, imax
        if (ndm(i,j,k) /= rmiss) then
          ...
          !c+++  airvol [m3] = area [m^2] * height [m]
          dout(i,j,k) = area * height
        endif
      enddo !! i
    enddo !! j
  enddo !! k
...
subroutine getparms
  ...
  !c+++ 
  call get_parms('item', 'AIRVOL', hitem, ios)
  call get_parms('titl', 'air volume', htitl, ios)
  call get_parms('unit', 'm|3"', hunit, ios)
  !c+++ start/end time
...
subroutine xabort
  write(6, '(a)') 'Usage: '
  write(6, '(a)') 'gtairvol -o output-file'
  ...
  write(6, '(a)') '------------------------------------------'
  write(6, '(a)') ' '
  write(6, '(a)') 'ex. gridbox volume for p-levels: '
  write(6, '(a)') '    gtairvol -i air_P -o airvol'
  stop 2
