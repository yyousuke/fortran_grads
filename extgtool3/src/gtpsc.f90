...
!c
!c  PSC area poleward of 60 (deg) at selected level.
!c
...
!c===
  ...
  do while(1 == 1)
    ...
    !c+++ PSC Area
    call psc_area(imax, jmax, kmax, nymin, nymax, lev, rmiss, d, pres, lat, psc_out)
...
!c=====
subroutine psc_area(imax, jmax, kmax, nymin, nymax, plev, rmiss, d, pres, lat, psc_out)
  use common_psc, only: a, h
  !c+++ [input]
  ...
  real(kind=r8b), intent(in)    :: d(imax,jmax,kmax)    !! input data (cm2/cm3)
...
&          abs(0.5d0 * (cos(xlatm(j)) + cos(xlatm(j+1))) * (xlatm(j) - xlatm(j+1)))
    do i = 1, imax
      ...
      enddo !!
      if (k_l /= 0.and.d(i,j,k) /= rmiss) then
        !c+++ height = (z(k+1)+z(k))/2 - (z(k)+z(k-1))/2
        height = - 0.5d0 * h * (log(pres(i,j,k_h)/1000.d0) - log(pres(i,j,k_l)/1000.d0))
        psc = psc + d(i,j,k_c) * 1.d2 * area * height !! (m2)
      endif
      !ccc write(6, *) 'height = ', height, -h*log(pres(i,j,k_h)/1000.d0), -h*log(pres(i,j,k_l)/1000.d0)
    enddo !! i
  enddo !! j
  psc_out = psc
...
subroutine xabort
  ...
  write(6, '(a)') '-i input-PSC-file -o output-file'
  write(6, '(a)') '(unit of input-file must be cm2/cm3)'
  write(6, '(a)') ' '
